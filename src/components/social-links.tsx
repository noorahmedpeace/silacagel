...
type Social = {
  name: string;
  href: string;
  /** Official brand glyph path (24x24 viewBox). */
  path: string;
};
...
type SocialLinksProps = {
  className?: string;
  /** Optional visible label above the icons. */
  label?: string;
};

export function SocialLinks({ className, label }: SocialLinksProps) {
  return (
    <div className={`${styles.wrap} ${className ?? ""}`.trim()}>
      ...
      <ul className={styles.row} aria-label="DryGelWorld profiles and stores">
        {SOCIALS.map((social) => (
          <li key={social.name}>
            <a
              className={`${styles.icon} ${styles[social.name.toLowerCase()]}`}
              href={social.href}
              target="_blank"
              rel="noopener noreferrer"
              aria-label={`DryGelWorld on ${social.name} (opens in a new tab)`}
              title={social.name}
            >
